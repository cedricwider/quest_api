...
setup_letsencrypt() {
  ...
  echo "ced.li questapi.ced.li" > domains.txt
...
create_pems() {
  openssl req -x509 -nodes -days 730 -newkey rsa:1024 -keyout privkey.pem -out fullchain.pem -subj "/C=CH/ST=ZH/L=Daellikon/O=1337_Inc./OU=Software/CN=questapi.ced.li"
...
if [ "$CA_SSL" = "true" ]; then
  ...
  cp $SSL_ROOT/certs/ced.li/fullchain.pem $SSL_ROOT/certs/ced.li/privkey.pem $SSL_CERT_HOME
